...
export const createActivitySchema = z.object({
  ...
  max_submissions_per_user: z.number().int().positive().optional().nullable(),

  total_available_slots: z.number().int().positive().optional().nullable(),
...
export const submitActivitySchema = z
  ...
  .refine(
    ...
    {
      message:
        'At least one of: submission_url, submission_text, or evidence_files must be provided',
    },
  )
...
export const createDistributionSchema = z
  .object({
    ...
    transaction_hashes: z
      .record(z.string().uuid(), z.string().regex(/^0x[a-fA-F0-9]{64}$/))
    ...
    (data) => {
      ...
      if (data.distribution_method === 'manual') {
        return (
          ...
          Object.keys(data.transaction_hashes).length > 0
        )
      }
      return true
    },
...
export const listSubmissionsQuerySchema = z.object({
  ...
  sort: z
    .enum(['submitted_at_asc', 'submitted_at_desc', 'reward_amount_desc'])

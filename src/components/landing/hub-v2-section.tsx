...
export default function HubV2Section() {
  const activities = [
    ...
    { icon: Coffee, label: 'Office hours con mentores', color: 'bg-muted' },
  ]
      ...
      <div className="page-content">
        <div className="mx-auto max-w-5xl space-y-16">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {activities.map((activity, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group relative flex flex-col items-center gap-3 overflow-hidden rounded-xl bg-white p-6 text-center shadow-lg transition-all hover:scale-105"
                >
                  <div className={`absolute inset-0 ${activity.color} opacity-0 transition-opacity group-hover:opacity-10`}></div>
          ...
          <div className="space-y-6 rounded-2xl bg-muted p-8 md:p-10 shadow-lg">
            <h3 className="text-center text-2xl font-black text-background md:text-3xl">
              ¿Eres{' '}
              <span className="inline-block rotate-1 transform rounded-lg bg-accent px-3 py-1 text-foreground shadow">
                Hacker?
              </span>
            </h3>
            <p className="text-center text-lg font-bold text-background/80">
              No hablamos de código. Hablamos de{' '}
              <span className="text-primary">mentalidad.</span>
            </p>
            ...
            <p className="text-center text-lg font-black text-background">

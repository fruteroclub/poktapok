...
export function EditableProfileCard({
  ...
  const [formData, setFormData] = useState({
    ...
    learningTrack:
      (profile?.learningTracks?.[0] as 'ai' | 'crypto' | 'privacy') || 'ai',
    availabilityStatus:
  ...
  const mutation = useMutation({
    ...
    onError: (error: Error) => {
      toast.error(
        error.message || `Failed to ${profile ? 'update' : 'create'} profile`,
      )
    },
  })
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    {currentAvailability && (
                      <>
                        <span
                          className={`h-2 w-2 rounded-full ${currentAvailability.color}`}
                        />

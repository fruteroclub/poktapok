...
```typescript
import { pgTable, uuid, varchar, text, real, integer, pgEnum, foreignKey, check, index } from 'drizzle-orm/pg-core'
...
async function testProfilesSchema() {
  ...
  console.log(`✅ Cascade delete works: ${remainingProfiles.length === 0 ? 'YES' : 'NO'}`)

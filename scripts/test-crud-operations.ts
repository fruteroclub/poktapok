...
async function testCRUDOperations() {
  ...
  try {
    ...
    try {
      await db.delete(applications).where(sql`user_id IN (SELECT id FROM users WHERE privy_did = 'did:privy:test_crud_user')`)
      await db.delete(invitations).where(sql`inviter_user_id IN (SELECT id FROM users WHERE privy_did = 'did:privy:test_crud_user')`)
      await db.delete(profiles).where(sql`user_id IN (SELECT id FROM users WHERE privy_did = 'did:privy:test_crud_user')`)
    ...
    const [foundProfile] = await db.select()
      .from(profiles)
    ...
    console.log('  ✅ User with profile joined:', userWithProfile.length, 'row(s)')
    ...
    const [updatedInvite] = await db.update(invitations)
      .set({
    ...
    console.log('  ✅ Soft deleted user hidden from active queries:', activeUsers.length === 0 ? 'YES' : 'NO')
    ...
    console.log('  ✅ Application deleted manually (FK is SET NULL but column is NOT NULL)')

    // Delete invitation (has CASCADE DELETE for inviter)
    const invitesBefore = await db.select().from(invitations).where(eq(invitations.inviterUserId, newUser.id))
    ...
    console.log('  ✅ CASCADE DELETE for profiles:', remainingProfiles.length === 0 ? 'YES (worked)' : 'NO (failed)')

    // Check invitations were cascade deleted
    const remainingInvites = await db.select()
      .from(invitations)
      .where(eq(invitations.inviterUserId, newUser.id))

    console.log('  ✅ CASCADE DELETE for invitations:', remainingInvites.length === 0 ? 'YES (worked)' : 'NO (failed)')

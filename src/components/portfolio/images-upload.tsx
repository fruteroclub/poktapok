...
export function ImagesUpload({
  ...
  const [images, setImages] = useState<ImageItem[]>(
    currentImageUrls.map((url) => ({ url, isUploading: false, isDeleting: false }))
  );
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        if (projectId) {
          ...
          setImages((prev) => {
            const withoutPreviews = prev.filter((img) => !previews.includes(img.url));
            const uploaded = uploadedUrls
              .slice(-compressedFiles.length)
              .map((url: string) => ({ url, isUploading: false, isDeleting: false }));
  ...
  const handleDelete = async (imageUrl: string, index: number) => {
    ...
    try {
      setImages((prev) => prev.map((img, i) => (i === index ? { ...img, isDeleting: true } : img)));

      // Encode URL as Base64 for safe URL path
      const encodedImageId = Buffer.from(imageUrl).toString('base64');
      const response = await fetch(`/api/projects/${projectId}/images/${encodeURIComponent(encodedImageId)}`, {
        method: 'DELETE',
      });
      ...
      setImages((prev) => prev.map((img, i) => (i === index ? { ...img, isDeleting: false } : img)));
    }
  };
  ...
  return (
    <div className="space-y-4">
          ...
          {images.map((image, index) => (
            <div
              ...
              className={`
                ...
                ${!image.isUploading && !image.isDeleting && !disabled ? 'cursor-move' : ''}
              `}
            >
              {/* Drag Handle */}
              ...
              <div className="aspect-square relative">
                <img src={image.url} alt={`Project image ${index + 1}`} className="w-full h-full object-cover" />
          ...
          <div className="flex flex-col items-center space-y-3 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                JPEG, PNG, WebP, SVG • Max 5MB each • {IMAGE_CONFIG.maxAdditionalImages - images.length} remaining
              </p>
            </div>
          </div>
        </div>
      ...
      <p className="text-xs text-muted-foreground">
        Add up to {IMAGE_CONFIG.maxAdditionalImages} additional images to showcase your project. Drag images to
        reorder.
      </p>
    </div>
  );

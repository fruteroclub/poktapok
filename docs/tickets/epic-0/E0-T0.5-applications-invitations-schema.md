...
```typescript
import { pgTable, uuid, varchar, text, timestamp, pgEnum, foreignKey, index } from 'drizzle-orm/pg-core'
...
```typescript
import { pgTable, uuid, varchar, timestamp, pgEnum, foreignKey, index, check } from 'drizzle-orm/pg-core'
import { sql } from 'drizzle-orm'
...
- User signs up → creates application with motivation text
- Admin reviews → approves/rejects with notes
...
async function testApplicationsInvitationsSchema() {
  console.log('🧪 Testing applications and invitations schemas...\n')

  // Create test users
  const [applicant] = await db.insert(users).values({
  ...
  const [reviewedApp] = await db.update(applications)
    .set({
      status: 'approved',
  ...
  const [redeemedInvite] = await db.update(invitations)
    .set({
  ...
  console.log('\n🎉 Applications and invitations schemas tests passed!')
}

...
/**
 * Luma Sync Action (Public)
 * 
 * Can be called from frontend or scripts for testing.
 * The cron job calls the internal mutation version (sync.ts)
 */
...
/**
 * Public action to sync Luma calendar
 */
export const syncCalendar = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      // 3. Upsert each event via mutation
      let created = 0;
      let updated = 0;

      for (const calendarEvent of calendarEvents) {
        try {
          const result = await ctx.runMutation(api.events.upsert, {

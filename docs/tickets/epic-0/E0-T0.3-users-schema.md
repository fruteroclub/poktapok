...
```typescript
import { pgTable, uuid, varchar, timestamp, pgEnum, jsonb, foreignKey, check, index } from 'drizzle-orm/pg-core'
...
async function testUsersSchema() {
  ...
  const [walletUser] = await db.insert(users).values({
  ...
  console.log('✅ Wallet user created:', walletUser.id)
  ...
  console.log('✅ App wallet added:', updatedUser.appWallet)

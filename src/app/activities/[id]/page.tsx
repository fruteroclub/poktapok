      ...
      <PageWrapper>
        <div className="page">
          <div className="page-content">
            <div className="py-12 text-center">Loading activity...</div>
          </div>
        </div>
      </PageWrapper>
    )
          ...
          <Card className="mb-8 w-full">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 border-t pt-4">
                <div>
                  <h4 className="mb-1 font-medium">Submissions</h4>
                ...
                {activity.maxSubmissionsPerUser && (
                  <div>
                    <h4 className="mb-1 font-medium">Max Per User</h4>
                    <p className="text-sm text-muted-foreground">
                      {activity.maxSubmissionsPerUser}
                    </p>
                  </div>
                ...
                {activity.expiresAt && (
                  <div>
                    <h4 className="mb-1 font-medium">Expires</h4>
                    <p className="text-sm text-muted-foreground">
              ...
              <div className="border-t pt-4">
                <h4 className="mb-3 font-medium">Required Evidence</h4>
          ...
          {canSubmit(activity) ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  {activity.evidenceRequirements.url_required && (
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        Provide a link to your work (GitHub commit, X post,
                        etc.)
                      </p>
                    </div>
                  )}
                  ...
                  {activity.evidenceRequirements.text_required && (
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        Explain your work and key learnings (
                        {formData.submission_text.length}/1000 characters)
                      </p>
                    </div>
                  )}
                  ...
                  {activity.evidenceRequirements.screenshot_required && (
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-amber-600">
                        Screenshot upload feature coming soon. For now, you can
                        include screenshot links in your description.
                      </p>
                    </div>
                  )}
            ...
            <Card className="w-full">
              <CardContent className="py-8 text-center text-muted-foreground">
                {isFull(activity) && (
                  <p>This activity has reached its maximum submissions.</p>
                )}
                {isExpired(activity) && <p>This activity has expired.</p>}
                {activity.status !== 'active' && (
                  <p>This activity is not currently accepting submissions.</p>
                )}
              </CardContent>
            </Card>
          )}
